...
export function ValueProps({ lang }: { lang: Lang }) {
  ...
  // hairlines (stacked on phones), each a coral icon chip + label.
  return (
    <section className="mt-6 sm:mt-8">
      <Reveal y={18}>
        <div className="grid grid-cols-1 sm:grid-cols-3 rounded-[1.6rem] border border-line bg-gradient-to-br from-[#FFF7F5] via-white to-white overflow-hidden">
          {ITEMS.map(({ Icon, k, s }, i) => (
            <div key={k} className={`group flex items-center gap-3 sm:gap-4 px-5 py-4 sm:px-6 sm:py-6 border-line ${
