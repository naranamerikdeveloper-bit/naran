        ...
        {ITEMS.map(({ Icon, k, s }, i) => (
          <Reveal key={k} delay={i * 0.07} y={22} className="h-full">
            <TiltCard className="group h-full flex items-center gap-3.5 sm:gap-4 bg-white border border-line rounded-[1.5rem] p-4 sm:p-5 elev-3d elev-3d-hover transition-colors duration-200 hover:border-accent/30">
              <span
                className="depth relative overflow-hidden w-12 h-12 sm:w-14 sm:h-14 rounded-2xl grid place-items-center text-accent-deep shrink-0 transition-colors duration-300 group-hover:text-white shadow-[inset_0_1px_0_rgba(255,255,255,.8),0_10px_20px_-10px_rgba(143,75,64,.45)]"
                style={{ background: "linear-gradient(150deg,#F6E4DF,#FFFFFF)" }}
              >
                <span className="hover-icon-bg absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: "linear-gradient(150deg,#D8A092,#8F4B40)" }} aria-hidden />
