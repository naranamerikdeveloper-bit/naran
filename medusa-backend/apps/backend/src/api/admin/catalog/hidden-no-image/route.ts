...
// Powers the product-list warning widget. Guarded by catalog.read (middlewares).
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const hidden = (data || []).filter((p: any) => p.metadata?.hidden_reason === HIDDEN_NO_IMAGE);
  res.json({
    count: hidden.length,
    sample: hidden.slice(0, 8).map((p: any) => ({ id: p.id, title: p.title })),
  });
}
